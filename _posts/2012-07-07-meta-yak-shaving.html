...
With software, this usually presents itself as spending <em>way</em> too much time refactoring without making meaningful forward progress.  Don't get me wrong - refactoring should be a daily activity to ensure that you don't get overwhelmed by mounting complexity, but when you're refactoring perfectly-fine code again and again because it's kinda fun to do so, you're yak shaving.<!--more-->
...
Another surprise: the 'else' keyword appears only 4 times in those 1432 lines.  Weird.  I certainly don't consciously think of 'else' as somehow <em>bad</em>.  The explanation comes in seeing how else conditional logic presents itself.

First, polymorphism hides some conditional logic, as it implies statements like "if the runtime type of x is T1 then call T1's implementation, <strong>else</strong> if the runtime type of x is T2 then call T2's implementation, <strong>else</strong>…"
...
The only interesting bit of mutation comes in the <a href="https://github.com/plioi/parsley/blob/2d113754b6e3e68a43e624987fb7ed5c4dd99870/src/Parsley/TokenStream.cs">TokenStream</a> class, which repeatedly mutates a <a href="http://www.headspring.com/blog/developer-deep-dive/public-private-super-private/">super-private</a> IEnumerator by calling its <code>MoveNext()</code> method.  Amusingly, this admittedly-complex class exists precisely so that you can traverse an IEnumerable in an apparently-mutation-free way.  My worst offender in the fight against mutable state exists in order to cover up some mutable state!
