...
The <code>yield</code> keyword allows us to produce results on demand.  The <code>Enumerable</code> extension methods <code>Zip</code>, <code>Skip</code>, and <code>TakeWhile</code> are all likewise implemented with the <code>yield</code> keyword.  We have effectively glued together a chain of function calls, each of which draws out the next result on demand.  In the main loop, TakeWhile repeatedly tries to pull the next item of the Fibonacci series.  When it does so, Fib() runs until the next time it yields a value, which in turn relies on summing up two <em>other</em> 'instances' of the Fibonacci sequence.  Since everything's lazy, none of this gets stuck in an infinite loop.

Haskell assumes you'll want to be lazy all the time.  The benefit is that when you do want laziness, it comes with brevity.  C# on the other hand offers you two tools for laziness when you really <em>want/need</em> it: lambdas and yield.